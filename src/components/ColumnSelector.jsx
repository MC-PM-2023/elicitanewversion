...
export default function ColumnSelector({
  ...
  return (
    <div
      className={`column-grid ${
        selectedColumn ? 'selection-active' : ''
      } grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-10 gap-4 mb-1  border border-gray-200 rounded-2xl shadow-md p-4 bg-white mb-4`}
    >
      {/* Multi-columns that open modal */}
      {/* {Object.keys(multiColumns).map((colName, idx) => (
        <motion.button
...
{columns.map((col, idx) => {
  ...
  return (
    <motion.button
      ...
      data-table={col.table}
      data-column={col.column}
      className="column-btn flex flex-col justify-center items-start text-left p-3 rounded-lg shadow-md transition-all"
      style={{
        backgroundColor: isSelected
          ? baseColor
          : related
